...
static bool ws_handshake(SOCKET s, const std::string &host, int port, const std::string &path)
{
    ...
    std::string rs = r.str();
    if (!send_all(s, rs.data(), (int)rs.size()))
        return false;
    std::string h;
    char ch;
    while (h.size() < 8192)
...
static int ws_recv(SOCKET s, std::vector<uint8_t> &payload)
{
    ...
    if (masked && recv_n(s, (char *)mk, 4) != 4)
        return -1;
    if (len > (1u << 20))
        return -1;
    payload.resize((size_t)len);
    if (len && recv_n(s, (char *)payload.data(), (int)len) != (int)len)
        return -1;
    if (masked)
    ...
    if (op == 0x9)
    {
        ws_send(s, 0xA, payload.data(), payload.size());
        return 0;
    }
    if (op == 0xA)
        return 0;
...
static void ctrl_send_hello(Runtime &rt)
{
    ...
    if (rt.ctrl_sock == INVALID_SOCKET)
        return;
...
static std::string build_ffmpeg_cmd(const Config &base, const Runtime &r)
{
    ...
    else
    {
        ...
        {
            // libx264: Main-профиль — широкая совместимость с WebCodecs-декодерами
            // (в т.ч. чистый software в VM). В High есть 8x8 transform и custom
            // quant matrices; на десктопном контенте без них качество почти то же.
            int gop = r.framerate * 2;
...
static void run_session(const Config &base, Runtime &rt)
{
    ...
    if (!send_all(sock, s.data(), (int)s.size()))
    {
        closesocket(sock);
        return;
    }
