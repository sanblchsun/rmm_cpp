...
static bool tls_handshake(TlsConn *c, const std::string &host, bool verify_cert)
{
    ...
    if (ss != SEC_I_CONTINUE_NEEDED) {
        std::ostringstream o; o << std::hex << (DWORD)ss;
        log("ISC initial failed: 0x" + o.str()); return false;
    }
    ...
    while (true) {
        // Читаем из сокета (нужно при CONTINUE_NEEDED и INCOMPLETE_MESSAGE)
        {
            ...
            if (n <= 0) { log("tls_handshake: socket closed"); return false; }
    ...
    retry:
        ...
        if (ss == SEC_I_CONTINUE_NEEDED)  continue;   // нужно больше данных от сервера
        if (ss == SEC_E_INCOMPLETE_MESSAGE) {           // нужно дочитать текущий пакет
            int n = recv(c->sock, tmp, sizeof tmp, 0);
            if (n <= 0) { log("tls_handshake: socket closed (incomplete)"); return false; }
            in_buf.insert(in_buf.end(), tmp, tmp + n);
            goto retry;
        }
        // Любая другая ошибка
...
static TlsConn *tls_connect(const std::string &host, int port, bool verify_cert)
{
    ...
    if (!tls_handshake(c, host, verify_cert)) {
        tls_close(c); delete c; return nullptr;
    }
    return c;
}
...
static int tls_recv_some(TlsConn *c, char *buf, int want)
{
    ...
    for (;;) {
        ...
        while (!c->raw.empty()) {
            ...
            if (ss == SEC_E_INCOMPLETE_MESSAGE) break; // нужно дочитать пакет
...
static bool send_chunk(TlsConn *c, const char *p, int n)
{
    ...
    if (!send_all(c, h, hl))             return false;
    if (n > 0 && !send_all(c, p, n))     return false;
    return send_all(c, "\r\n", 2);
}
                             ...
                             const std::string &path, bool verify_cert)
{
    ...
    if (!send_all(c, req.data(), (int)req.size())) { tls_close(c); delete c; return {}; }
...
static bool json_str_ex(const std::string &j, const std::string &k, std::string &out)
{
    ...
    while (p < j.size()) {
        ...
        if (c == '\\' && p + 1 < j.size()) {
            ...
            if (n=='u' && p+5 < j.size()) {
                unsigned cp = 0;
                for (int i=0;i<4;++i){unsigned v;if(!hex(j[p+2+i],v))return false;cp=(cp<<4)|v;}
                p+=6;
                if (cp>=0xD800&&cp<=0xDBFF&&p+5<j.size()&&j[p]=='\\'&&j[p+1]=='u') {
                    unsigned low=0; bool ok=true;
                    for(int i=0;i<4;++i){unsigned v;if(!hex(j[p+2+i],v)){ok=false;break;}low=(low<<4)|v;}
                    if(ok&&low>=0xDC00&&low<=0xDFFF){cp=0x10000+((cp-0xD800)<<10)+(low-0xDC00);p+=6;}
                }
                emit_cp(cp); continue;
            }
            return false;
...
static bool read_screen_metrics(int &w, int &h, int &ox, int &oy)
{
    ...
    ox=GetSystemMetrics(SM_XVIRTUALSCREEN); oy=GetSystemMetrics(SM_YVIRTUALSCREEN);
    if(w<=0||h<=0){w=GetSystemMetrics(SM_CXSCREEN);h=GetSystemMetrics(SM_CYSCREEN);ox=0;oy=0;}
    return w>0&&h>0;
}
static void init_screen_metrics()
{
    ...
    if(read_screen_metrics(w,h,ox,oy)){
        g_screen_w=w; g_screen_h=h; g_screen_origin_x=ox; g_screen_origin_y=oy;
    }
...
static void do_mouse_move(int x, int y)
{
    ...
    if(sw<=1||sh<=1)return;
    if(x<0)x=0;if(y<0)y=0;if(x>=sw)x=sw-1;if(y>=sh)y=sh-1;
...
static void do_mouse_button(int button, bool down)
{
    ...
    in.mi.dwFlags=f; SendInput(1,&in,sizeof(INPUT));
}
...
static void do_text_input(const std::string &utf8)
{
    ...
    std::vector<INPUT> inputs; inputs.reserve(w.size()*2);
    auto push_vk=[&](WORD vk){
        INPUT d{};d.type=INPUT_KEYBOARD;
        d.ki.wVk=vk; d.ki.wScan=(WORD)MapVirtualKeyW(vk,MAPVK_VK_TO_VSC); d.ki.dwFlags=0;
    ...
    for(wchar_t ch:w){
        ...
        INPUT d{};d.type=INPUT_KEYBOARD;
        d.ki.wVk=0; d.ki.wScan=(WORD)ch; d.ki.dwFlags=KEYEVENTF_UNICODE;
...
{
    if(code.size()==4&&code.compare(0,3,"Key")==0){char c=code[3];if(c>='A'&&c<='Z')return c;}
    if(code.size()==6&&code.compare(0,5,"Digit")==0){char c=code[5];if(c>='0'&&c<='9')return c;}
    ...
    auto it=m.find(code); return it==m.end()?0:it->second;
}
static void do_key(const std::string &code, bool down)
{
    int vk=code_to_vk(code); if(vk==0)return;
    ...
    switch(vk){
        ...
        case VK_LWIN:case VK_RWIN:case VK_NUMLOCK:
            in.ki.dwFlags|=KEYEVENTF_EXTENDEDKEY; break;
    }
    SendInput(1,&in,sizeof(INPUT));
}
...
{
    bool opened=false;
    for(int i=0;i<10;++i){if(OpenClipboard(NULL)){opened=true;break;}Sleep(15);}
    if(!opened)return {};
    std::string result;
    ...
    if(h){
        ...
        if(w){
            ...
            if(n>1){result.resize((size_t)n-1);WideCharToMultiByte(CP_UTF8,0,w,-1,&result[0],n,NULL,NULL);}
            GlobalUnlock(h);
        }
    }
                         ...
                         const std::string &path)
{
    ...
    std::string rs=r.str();
    if(!send_all(c,rs.data(),(int)rs.size()))return false;
    std::string h; char ch;
    while(h.size()<8192){
        if(recv_n(c,&ch,1)!=1)return false;
        h+=ch;
...
{
    uint8_t h[2]; if(recv_n(c,(char*)h,2)!=2)return -1;
    int op=h[0]&0x0F; bool masked=(h[1]&0x80)!=0;
    uint64_t len=h[1]&0x7F;
    if(len==126){
        uint8_t b[2];if(recv_n(c,(char*)b,2)!=2)return -1;
        len=((uint64_t)b[0]<<8)|b[1];
    }else if(len==127){
        uint8_t b[8];if(recv_n(c,(char*)b,8)!=8)return -1;
        len=0;for(int i=0;i<8;++i)len=(len<<8)|b[i];
    }
    uint8_t mk[4]={0,0,0,0};
    if(masked&&recv_n(c,(char*)mk,4)!=4)return -1;
    if(len>(8u<<20))return -1;
    payload.resize((size_t)len);
    if(len&&recv_n(c,(char*)payload.data(),(int)len)!=(int)len)return -1;
...
static void control_loop(const Config &cfg, Runtime &rt)
{
    while(!rt.stop){
        ...
        { std::lock_guard<std::mutex> lk(rt.ctrl_sock_m); rt.ctrl_conn=c; }
        ctrl_send_hello(rt);
        ...
        { std::lock_guard<std::mutex> lk(rt.ctrl_sock_m); rt.ctrl_conn=nullptr; }
        tls_close(c); delete c;
...
static void run_session(const Config &base, Runtime &rt)
{
    ...
    if(!c){ log("tls_connect failed"); return; }
    ...
    if(!send_all(c,s.data(),(int)s.size())){tls_close(c);delete c;return;}
    ...
    if(!pipe){tls_close(c);delete c;return;}
    ...
    while(!rt.stop){
        ...
        if(!send_chunk(c,buf.data(),(int)n)){log("send failed");break;}
        bytes+=n;
...
static void parse_cli(Config &c, int argc, char **argv)
{
    for(int i=1;i<argc;++i){
        std::string a=argv[i];
        // Флаги без значения
        if(a=="--insecure"){ c.verify_cert=false; continue; }
        if(a=="--https" || a=="--https=true"){ /* уже по умолчанию 443 порт */ continue; }
        ...
        else if(k=="--https")   { if(v=="false") c.server_port = 8000; } // HTTP fallback
    }
}
...
int main(int argc, char **argv)
{
    ...
    WSADATA w;
    if(WSAStartup(MAKEWORD(2,2),&w)!=0){log("WSAStartup failed");return 1;}
